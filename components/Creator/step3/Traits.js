    ...
    return(
        ...
        >
            <m.div layout className="my-2 flex justify-between items-center w-full">
                <m.div layout className="p-2 bg-lightGrayCube relative   rounded-xl w-48 text-center flex items-center justify-around ">
                ...
                <m.div id={`Container_${traitName}`} layout className={`flex w-80  p-2 mx-1 border-2 border-inputColor rounded-2xl `}>
                ...
                <m.input disabled={isOnly} onChange={(e) => handleInputChangeTrait(e, setInputValue, setInputValueInt, traitName)} value={inputValue} className={` border-2 border-lightGray bg-transparent rounded-3xl w-34 p-2 pl-4 ${isOnly ? "opacity-90 " : "border-lightGray"}`} placeholder={`${placeholderValue}`} type="text" />
