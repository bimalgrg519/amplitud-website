...
const generatePreview = (
  traitsAndAttributes: ITraitDetail[],
  traitsOrder: TTrait[],
  rule: IRules,
  tokenCount: number
) => {
  ...
  traitsOrder.forEach((traitName: TTrait) => {
    mapAttributes[traitName] = traitsAndAttributes
      .find((e) => {
        return e.id === traitName;
      })
      .attributes.map((e) => e.id);
  });

  console.log("mapAttributes", mapAttributes);

  const pickAttribute = (traitName: TTrait) => {
  ...
  while (generated.length < tokenCount && attempts < maxAttempts) {
    ...
    traitsOrder.forEach((traitName: TTrait) => {
      ...
      if (attributeName) {
        const file = traitsAndAttributes
          .find((e) => {
            return e.id === traitName;
          })
          .attributes.find((e) => e.id === attributeName).file;
  ...
  if (generated.length < tokenCount) {
    throw new Error(
      "Couldn't generate " +
        tokenCount +
        " NFTs. Please add more traits, reduce the number of NFTs or revise the combinations/exclusions."
    );
  }
...
const generateNFTs = (
  ...
  traitsOrder.forEach((traitName: TTrait) => {
    mapAttributes[traitName] = Object.keys(
      traitsAndAttributes[traitName].attributes
    );
    ...
    mapAttributes[traitName].forEach((attributeName: string) => {
      ...
      if (attribute.onlyone) {
        ...
        mapRarities[traitName].attributes.push({
...
const getTokenImages = (
  ...
  tokens.forEach((token: IToken) => {
    ...
    traitsOrder.forEach((traitName) => {
      ...
      if (attributeName) {
        let attribute =
          traitsAndAttributes[traitName].attributes[attributeName];
